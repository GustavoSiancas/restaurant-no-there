...
func (h *Handler) ListOrders(c *gin.Context) {
	orders, err := h.service.ListOrders(c, domain.ClaimStatus(c.DefaultQuery("status", string(domain.ClaimPending))))
	if err != nil {
...
func (h *Handler) ValidateOrder(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, core.ErrConflict) {
			...
			if order != nil && order.Status == domain.ClaimNotClaimed {
				code, message = "MEAL_ORDER_NOT_CLAIMED", "el trabajador no reclamó esta comida dentro del horario"
			}
