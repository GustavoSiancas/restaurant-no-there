...
func (r *Repository) CountDailyMealStatusRows(ctx context.Context, date time.Time, mealType domain.MealType) (int64, error) {
	var total int64
	err := r.db.QueryRow(ctx, `SELECT COUNT(*) FROM meal_claims
		WHERE service_date=$1::date AND meal_type=$2::meal_type`, date, mealType).Scan(&total)
	return total, err
}

func (r *Repository) DailyMealStatusRows(ctx context.Context, date time.Time, mealType domain.MealType, limit, offset int) ([]domain.DetailedReportRow, error) {
	...
	JOIN user_credentials dni ON dni.user_id=c.worker_id AND dni.type='DNI' AND dni.active=TRUE
	JOIN worker_information wi ON wi.user_id=c.worker_id
	WHERE c.service_date=$1::date AND c.meal_type=$2::meal_type
	ORDER BY c.created_at,c.id
	LIMIT $3 OFFSET $4`, date, mealType, limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
